...
import { findProductsSearch } from "../services/apiClient";
                    ...
                    {searchResults.length > 0 &&
                        <div className="absolute left-0 right-0 top-full mx-auto w-full max-w-2xl bg-white dark:bg-neutral-800 shadow-xl rounded-b-xl z-50 max-h-192 overflow-y-auto border-t border-neutral-200 dark:border-neutral-700">
                            {searchResults.map((product: Product, index) => (
                                <div key={index} onClick={() => router.push(`/product/${product._id}`)} className="flex items-center gap-4 border-b dark:border-neutral-700 last:border-b-0 py-3 px-4 cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-700 transition">
