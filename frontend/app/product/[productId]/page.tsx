...
import AboutCard from '@/app/components/AboutInfoCard';
import { errorLog, log } from '@/app/lib/logger';
import { useCart } from '@/app/context/CartContext';
import Loading from '@/app/components/Loading';
import { fetchProductById } from '@/app/services/apiClient';
...
export default function ProductDetails() {
    ...
    const getProducts = async () => {
        ...
        setLoading(true)
        try {
            const data = await fetchProductById(productId) as Product
            setProduct(data)
        } catch (error) {
        ...
        <div className="min-h-screen flex flex-col font-montserrat bg-[#faf8f6] dark:bg-neutral-900">
                ...
                <div className='flex flex-col max-w-md pl-5 justify-center'>
                    ...
                    <div className='flex flex-wrap gap-2 py-2 items-center'>
                        {sizes.map((item, index) =>
                            <button key={index} onClick={() => setSize(item)} value={item} className={`bg-gray-200 dark:bg-neutral-600 dark:text-neutral-100 px-3 py-1 rounded transition-colors duration-300 ease-in-out cursor-pointer ${size === item ? "bg-gray-500 dark:bg-neutral-800" : ""} disabled:cursor-not-allowed disabled:opacity-50`} disabled={product.availability === "out"}>
                                {item}
                            </button>
                        )}
                    </div>
            ...
            <div className="max-w-5xl mx-auto mt-10 p-6 bg-white dark:bg-neutral-800 rounded shadow flex flex-col">
                <div className="flex gap-2 mb-6 border-b border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 relative">
                    <button onClick={() => setActiveTab("description")} value="description" className={`group relative px-6 py-3 text-xl font-semibold focus:outline-none transition-all cursor-pointer ${activeTab === "description" ? "text-[#c1a875]" : "text-gray-500 dark:text-neutral-300 hover:text-[#c1a875]"}`} style={{ minWidth: 120 }}>
                        Description
                        {activeTab === "description" &&
                            <span className="absolute left-4 right-4 bottom-[-2px] h-[3px] bg-[#c1a875] rounded-full" style={{ zIndex: 2 }} />
                        }
                    </button>
                    <button onClick={() => setActiveTab("reviews")} value="reviews" className={`group relative px-6 py-3 text-xl font-semibold focus:outline-none transition-all cursor-pointer ${activeTab === "reviews" ? "text-[#c1a875]" : "text-gray-500 dark:text-neutral-300 hover:text-[#c1a875]"}`} style={{ minWidth: 120 }}>
                        Reviews
                        {activeTab === "reviews" &&
                            <span className="absolute left-4 right-4 bottom-[-2px] h-[3px] bg-[#c1a875] rounded-full" style={{ zIndex: 2 }} />
                        }
