...
import { fetchProductsByIds } from '../services/apiClient';
    ...
    const getProductsByIds = async () => {
        ...
        try {
            ...
            const items = cart.map(cartItem => { // get the product from the localStorage
                const product = data.find((item: { _id: string }) => item._id === cartItem.id)
                return product
                    ? {
                        ...product,
                        id: product._id,
                    ...
                    {loading
                        ...
                        : fullCart.length === 0 ? (
                            <div className="bg-white/80 dark:bg-neutral-800/80 rounded-2xl shadow-md py-12 flex flex-col items-center">
                                ...
                                <button onClick={() => router.push("/")} className="px-8 py-3 border border-[#1a1a1a] bg-[#1a1a1a] text-white hover:bg-white hover:text-black transition rounded-2xl shadow font-semibold text-lg active:scale-95 mt-6 cursor-pointer">
                                    Shop Now
                                </button>
                            </div>
                        ) : (fullCart.map(item => (
                ...
                {fullCart.length > 0 && (
                    <div className="mt-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div className="flex flex-col md:items-end gap-2">
                            ...
                            <button onClick={() => router.push("/checkout")} className="w-full md:w-auto px-10 py-4 border bg-[#1a1a1a] text-white hover:bg-white hover:text-[#1a1a1a] transition rounded-2xl shadow-md font-semibold font-montserrat text-lg active:scale-95 mt-2 cursor-pointer">
                                Checkout
                            </button>
                        </div>
                    </div>
