...
const baseApiUrl = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api`
...
export const handleRegister = async (userData: { name: string; email: string; password: string }) => {
...
// Passive authentication before access the user panel - no token redirects to home
...
export const findProductsQuery = async (query: string) => {
    if (!query || query.trim() === "") {
...
export const handleGoogle = async (token: string) => {
...
export const fetchProductsByIdsOrders = async (ids: string[]) => {
    if (!Array.isArray(ids) || ids.length === 0) {
        log("Product ids are required")
        return
    }
