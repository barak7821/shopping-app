...
import { fetchOrdersByQuery, fetchProductsByIdsOrders } from '../services/apiClient'
...
export default function Orders() {
    ...
    const getOrders = async (signal: AbortSignal) => {
        ...
        setLoading(true)
        try {
            const data = await fetchOrdersByQuery(query, { signal }) as { items: Order[]; totalPages: number }
            if (signal.aborted) return
            setOrdersList(data.items)
            setTotalPages(Math.max(1, +data.totalPages || 1))

            const ids = data.items.map(item => item.orderItems.map(orderItem => orderItem.itemId)).flat()

            const products = await fetchProductsByIdsOrders(ids) as Product[]
    ...
    const upperCaseFormat = (value: string) => value.trim().length === 0 ? "Unknown" : value.replace(/\b\w/g, l => l.toUpperCase())
                            ...
                            <div key={item._id} className="bg-white/90 dark:bg-neutral-800/90 rounded-2xl shadow-xl p-7 flex flex-col gap-5">
                                ...
                                <div className="flex flex-col gap-3 divide-y divide-gray-100 dark:divide-neutral-700">
                                    {item.orderItems.map(item => {
                                        ...
                                        return (
                                            <div key={`${item.itemId}-${item.selectedSize}`} className="flex gap-4 py-3 items-center">
                                                <img src={product?.image || "Unknown"} alt={upperCaseFormat(item.itemTitle || product?.title || "Unknown")} className="w-28 h-28 object-cover rounded-xl border border-[#f2e8db] dark:border-neutral-600 shadow-sm" />
                                                <div className="flex flex-col gap-0.5">
                                                    <span className="font-medium text-base dark:text-neutral-100">
                                                        {upperCaseFormat(item.itemTitle || product?.title || "Unknown")}
                                                    </span>
                                ...
                                {/* More Details Button */}
                                <div className="flex justify-end mt-2">
                                    <button onClick={() => router.push(`/orders/${item.orderNumber}`)} className="px-6 py-2 rounded-2xl bg-[#1a1a1a] text-white border border-[#1a1a1a] font-semibold text-base shadow-md transition hover:bg-white hover:text-black active:scale-95 cursor-pointer">
                                        More Details
                                    </button>
                                </div>
                            </div>
