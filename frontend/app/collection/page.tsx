...
import { fetchProductsByQuery } from '../services/apiClient';
...
export default function Collection() {
    ...
    const getProducts = async (signal: AbortSignal) => {
        // Build query object
        const query: Query = { page: currentPage }
    ...
    const handlePageChange = (nextPage: number) => {
        ...
        const params = new URLSearchParams(searchParams.toString()) // Get current query params
    ...
    const handleParamChange = (paramKey: string, currentValues: string | string[], e: ChangeEvent<HTMLInputElement | HTMLSelectElement>, formatValue: (value: string) => string = v => v) => {
        const { value } = e.target
        const checked = "checked" in e.target ? e.target.checked : undefined // Get checked status if it's a checkbox
        ...
        if (typeof checked === "boolean") { // If the input is a checkbox
            ...
            const updatedValues = checked ? [...currentArray, formattedValue] : currentArray.filter(v => v !== formattedValue)
        ...
        router.replace(queryString ? `/collection?${queryString}` : "/collection")
                    ...
                    <div className="flex w-full gap-12">
                                    ...
                                    <div className="flex flex-col gap-2 text-sm text-[#555] dark:text-neutral-300">
                                        {[
                                            ...
                                            <label key={item.value} className="flex items-center gap-2 cursor-pointer">
                                                <input type="checkbox" value={item.value} className="accent-[#c1a875] w-4 h-4" checked={categories.includes(item.value)} onChange={e => handleParamChange("category", categories, e, v => v.toLowerCase())} />
                                                {item.name}
                                            </label>
                                        )}
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-col gap-2 text-sm text-[#555] dark:text-neutral-300">
                                        {[
                                            ...
                                            <label key={item.value} className="flex items-center gap-2 cursor-pointer">
                                                <input type="checkbox" value={item.value} className="accent-[#c1a875] w-4 h-4" checked={type.includes(item.value)} onChange={e => handleParamChange("type", type, e, v => v.toLowerCase())} />
                                                {item.name}
                                            </label>
                                        )}
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-col gap-2 text-sm">
                                        ...
                                        {[
                                            ...
                                            <label key={item.value} className="flex items-center gap-2 cursor-pointer text-[#555] dark:text-neutral-300">
                                                <input type="checkbox" value={item.value} className="accent-[#c1a875] w-4 h-4" checked={sizes.includes(item.value)} onChange={e => handleParamChange("sizes", sizes, e, v => v.toUpperCase())} />
                                                {item.name}
                                            </label>
                                        )}
                                    </div>
                                    ...
                                    <div className='flex justify-center'>
                                        <button
                                            ...
                                            className="text-sm text-[#c1a875] hover:underline mt-2 cursor-pointer"
                                        >
                                            Clear Filters
                                        </button>
                                    </div>
                        ...
                        <main className="flex-1 flex flex-col gap-6">
                            ...
                            <div className="flex justify-end mb-2">
                                <select onChange={e => handleParamChange("sort", sortBy, e, v => v.toLowerCase())} value={sortBy} className="rounded-2xl border border-gray-200 dark:border-neutral-700 px-4 py-2 text-base bg-neutral-50 dark:bg-neutral-800 shadow-sm focus:ring-2 focus:ring-[#c1a875] focus:outline-none w-auto dark:text-white">
                                    {[
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-12 gap-y-12">
                                    {loading
                                        ...
                                        : productsList.map(item =>
                                            <div key={item._id} onClick={() => router.push(`/product/${item._id}`)} className="flex flex-col items-center group cursor-pointer">
