...
import { handleGoogle, handleLogin } from '../services/apiClient';
...
export default function Login() {
    ...
    const login = useGoogleLogin({
        onSuccess: async (TokenResponse) => {
            ...
            try {
                ...
                if (redirect) {
                    router.push(redirect) // Navigate to the specified route
    ...
    const handleClick = async () => {
        ...
        try {
            ...
            const apiError = handleApiError(error, "handleClick")
            const code = apiError?.code

            if (code === "invalid_pass") setError("password")
        } finally {
            setLoading(false)
        }
    }
        ...
        <div className='min-h-screen bg-[#faf8f6] dark:bg-neutral-900'>
            ...
            <div className='flex justify-center items-center min-h-screen'>
                <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-xl p-8 md:p-12 w-full max-w-md flex flex-col gap-7 items-center">
                    ...
                    {/* Reset Password */}
                    <Link href={"/resetPassword"} className='text-sm text-[#80715a] hover:text-[#c1a875] hover:underline transition mt-1'>
                        Forgot your password?
                    </Link>

                    {/* Register */}
                    <Link href={"/register"} className='text-sm text-[#80715a] hover:text-[#c1a875] hover:underline transition mt-1'>
