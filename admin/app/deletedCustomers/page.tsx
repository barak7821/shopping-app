...
export default function DeletedCustomers() {
  ...
  const getUsers = async (signal?: AbortSignal) => {
    ...
    try {
      ...
      if ((error as any)?.response?.data?.code === "page_not_found") {
        router.replace("/deletedCustomers")
        return
      }
  ...
  const handlePageChange = (page: number) => {
    ...
    router.replace(`/deletedCustomers?${params.toString()}`)
                    ...
                    <tr key={user._id} className="hover:bg-[#faf8f6] dark:hover:bg-neutral-700/60 transition">
                      ...
                      <td className="px-6 py-4 border-t border-[#eee] dark:border-neutral-700">
                        <div className="flex items-center gap-3">
                          <button onClick={() => router.push(`/deletedCustomers/edit/${user._id}`)} title="Edit" className="p-2 rounded-full hover:bg-[#c1a875]/10 text-[#c1a875] transition cursor-pointer">
